...
/*获取login*/
//微博
if (isset($_GET['state']) && strlen($_GET['state']) > 10) {
    $ackey = substr($_GET['state'],0,strlen($_GET['state'])-2);
    $login = substr($_GET['state'],-2);
}
//微信、QQ
if (isset($_GET['login'])) {
    $login = $_GET['login'];
...
/*获取候选人数据*/
$can_json = $ac->getAllCandidateInfo($ackey);
...
if (isset($login)) {
    if ($login == 'wx') {
        $plat = '1';
        $userInfo = $wx->getUserInfoByCode();
        if (strlen($userInfo) < 200) {
            echo '<script>alert("身份认证错误！");</script>';
        }else{
            $userInfoArr = json_decode($userInfo, 1);
            $openId   = $userInfoArr["openid"];
            $nickName = $userInfoArr["nickname"];
            $imgUrl   = $userInfoArr["headimgurl"]; 
            /*用户添加到用户表*/
            $user = new User($openId, $nickName, $ackey, $plat, $userInfo);
            /*信息存入session*/
            ...
            $_SESSION["plat"]     = $plat;
            /*跳转*/
            header("Location: http://new.weixin.sky31.com/SkyVote/VotePages/index.php?ackey=$ackey") ;  
        }

    }elseif($login == 'wb'){
          $plat = '2';
          $userInfo = $wb->getUserInfo();
          $userInfoArr = json_decode($userInfo, 1);
          $openId     = $userInfoArr['id'];
          $nickName   = $userInfoArr['screen_name'];
          $imgUrl     = $userInfoArr['avatar_hd'];
          $user = new User($openId, $nickName, $ackey, $plat, $userInfo);
          /*信息存入session*/
          $_SESSION["openId"]   = $openId;
          $_SESSION["nickName"] = $nickName;
          ...
          $openId     = $userInfoArr['openid'];
          $nickName   = $userInfoArr['nickname'];
          $imgUrl     = $userInfoArr['figureurl_qq_2'];
          $user = new User($openId, $nickName, $ackey, $plat, $userInfo);
          /*信息存入session*/
          $_SESSION["openId"]   = $openId;
          $_SESSION["nickName"] = $nickName;
...
<html lang="zh-CN">
  <head>
    ...
    <title><?php echo $acInfo_array['activity_name']; ?>——SkyVote</title>
    <!-- mui-0.9.15-css -->
    ...
    <script>
        $(document).ready(function(){
            /*lazyload*/
            $("img.one-img").lazyload({ threshold : 100 ,effect : "fadeIn", failurelimit : 10});
            $("img.two-img").lazyload({ threshold : 100 ,effect : "fadeIn", failurelimit : 10});
        });
    </script>
    <!-- Font Awesome 4.7.0 -->
        ...
        <div id="container" class="mui--z3">
            ...
            <div id="ac-info">
                ...
                <table>
                    <tbody>
                        <tr>
                            <td style="border-right: 1px dashed #ECF0F1;">报名人数<div class="number"><?php echo count($can_array); ?></div></td>
                        ...
                        if ($key%2 == 0) {
                            if ($value['type'] == 0) {
                                ...
                                $html = '<div class="img-block mui--z3">
                                            <a href="http://'.Location.'/VoteAdmin/pages/candidate/img/img/'.$imgurl_array[0].'"><img class="can-img one-img" data-original="http://'.Location.'/VoteAdmin/pages/candidate/img/img/'.$imgurl_array[0].'"></a>
                                            <div class="img-info">
                                ...
                                $html = '<div class="img-block mui--z3">
                                            <a href="'.$value['linkurl'].'"><img class="can-img" data-original="http://'.Location.'/VoteAdmin/pages/candidate/img/cover/'.$linkcover.'">
                                            <div class="img-info">
                                                <div class="can-title p-limit">'.$value['name'].'</div>
                                                <div class="can-intro p-limit"><span class="votes" cankey='.$value['uniquekey'].'>'.$value['votes'].'</span>票&nbsp;&nbsp;&nbsp;'.$value['introduction'].'</div>
                                            </div>
                                            <div class="img-btn" style="overflow: hidden;">
                                                <a href="'.$value['linkurl'].'"><div class="btn-link mui--pull-left mui--text-center" linkurl='.$value['linkurl'].'>查看更多</div></a>
                                                <div class="btn-vote mui--pull-right mui--text-center" cankey='.$value['uniquekey'].'>投票</div>
                                            </div>
                                        </div>';
                                echo $html; 
                    ...
                    foreach ($can_array as $key=>$value) {
                        if ($key%2 != 0) {
                            if ($value['type'] == 0) {
                                ...
                                $imgurl_array = json_decode($imgurl, 1);
                                $html = '<div class="img-block mui--z3">
                                            <a href="http://'.Location.'/VoteAdmin/pages/candidate/img/img/'.$imgurl_array[0].'"><img class="can-img two-img" data-original="http://'.Location.'/VoteAdmin/pages/candidate/img/img/'.$imgurl_array[0].'"></a>
                                            <div class="img-info">
                                                <div class="can-title p-limit">'.$value['name'].'</div>
                                                <div class="can-intro p-limit"><span class="votes" cankey='.$value['uniquekey'].'>'.$value['votes'].'</span>票&nbsp;&nbsp;&nbsp;'.$value['introduction'].'</div>
                                            </div>
